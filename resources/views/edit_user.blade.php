...
<div style="background-color: #C8A77A; min-height: 80vh; padding: 40px;">
    <h1 style="text-align: center; color: #3E2723; margin-bottom: 30px;">Edit Data Mahasiswa</h1>
    ...
    <div style="max-width: 400px; margin: 0 auto; background-color: #D2B48C; padding: 20px; border-radius: 10px;">
        <form action="{{ route('user.update', $user->id) }}" method="POST">
            ...
            <div style="margin-bottom: 15px;">
                ...
                <input type="text" name="nama" id="nama" required value="{{ old('nama', $user->nama) }}"
            ...
            <div style="margin-bottom: 15px;">
                ...
                <input type="text" name="nim" id="nim" required value="{{ old('nim', $user->nim) }}"
                    style="width: 100%; padding: 8px; border: 1px solid #3E2723; border-radius: 4px;">
            </div>
            ...
            <div style="margin-bottom: 15px;">
                ...
                <select name="kelas_id" id="kelas_id" required
                    ...
                    <option value="A" {{ old('kelas_id', $user->kelas_id) == 'A' ? 'selected' : '' }}>A</option>
                    <option value="B" {{ old('kelas_id', $user->kelas_id) == 'B' ? 'selected' : '' }}>B</option>
                    <option value="C" {{ old('kelas_id', $user->kelas_id) == 'C' ? 'selected' : '' }}>C</option>
                    <option value="D" {{ old('kelas_id', $user->kelas_id) == 'D' ? 'selected' : '' }}>D</option>
                </select>
            </div>
